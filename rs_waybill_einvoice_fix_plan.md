...
**Date:** 2026-09-18 · **Updated:** 2026-09-25 · **Sources:** two independent source reviews (Claude, Codex), originally checked against Odoo 19, with Odoo 20 deltas below. **Status:** 52 numbered findings remain for review and implementation (#11, #13–#29, #31–#64). Confirmed resolved issues are removed; since 2026-09-25 numbers are no longer renumbered, so a gap is a removed item (#1–#9, #12 and #30 resolved, #10 dropped). Other open follow-ups and verification limits are noted separately.
...
journal hash mode and all five lock dates unchanged (#37, #41).

### D. Not re-verified

Items **#35, #38–#57, #59–#64** were re-checked only where they cite a core file (all such citations are
...
Current code ([_rs_auto_cancel_after_rs_cancellation](../custom_addons/gec_odoo_modules/rs_einvoice/models/account_move.py)): a document whose `payment_state` is not `not_paid` is left posted, gets one TODO and `rs_einv_reversal_pending_at`, and every Status Update retries (7 and 8 are checked on every refresh, and the recovery cron covers both). The manual path is unreconcile → Status Update → re-apply the payment to the replacement. A Skip-rs.ge credit note is not possible there today: `refund_moves` (the reversal wizard) and `_post` both refuse a credit note on an original that has a replacement. #37, #38, #40 and #41 change this same decision; design them together.
...
| 37 | P2 | `rs_einvoice (buyer)` | accounting | Accepting a supplier cancellation on a draft bill posts it first, then tries to cancel; hash-locked purchase journal leaves the void bill posted | Not exercised (test DB) |
| 38 | P2 | `rs_einvoice (buyer)` | accounting | Reject and Accept-cancellation paths ignore reversal_move_ids / payment_state 'reversed'; auto-cancel strands a reconciled credit note | Not exercised (test DB) |
| 39 | P3 | `rs_einvoice (buyer)` | accounting | Bills created from rs.ge (inbox, cron, pull) carry no purchase order link; PO stays 'To Bill' beside a posted bill | Not exercised (test DB) |
| 40 | P1 | `rs_einvoice (seller)` | accounting | Any credit note (draft, cancelled, Skip-rs.ge partial) permanently disables the local cancel at rs.ge status 7 and 8 - silently, no TODO | Reproduced (test DB) |
| 41 | P1 | `rs_einvoice (seller)` | accounting | Auto-cancel of a prior-period original rewrites an already declared VAT period (no lock dates in live DB) | Reproduced (test DB) |
...
| 43 | P2 | `rs_einvoice (seller)` | accounting | Sent (Pending) invoice reset to draft, buyer confirms meanwhile, re-post succeeds with edited lines; rs.ge keeps the confirmed old lines | Not exercised (test DB) |
| 44 | P3 | `rs_einvoice (seller)` | accounting | rs.ge status 7 written by _rs_apply_status_update never triggers the local cancel | Reproduced (test DB) |
...
| 46 | P1 | `rs_waybill + rs_einvoice (purchase bridge)` | purchase | Reimport Lines strips purchase_line_id from PO-created bill lines; PO drops to Waiting Bills while the bill stays posted | Not exercised (test DB) |
...
| 51 | P2 | `rs_waybill + rs_einvoice (purchase bridge)` | purchase | Bill pulled from rs.ge is never linked to the PO the module built from the same waybill | Not exercised (test DB) |
...
### 37. Accepting a supplier cancellation on a draft bill posts it first, then tries to cancel; hash-locked purchase journal leaves the void bill posted [C]
- **Module.** `rs_einvoice (buyer)`
...
- **Fix.** Branch on target_status before touching state: for target 7 do not post; call button_cancel() on the draft directly (core 6126-6138 handles drafts) after the rs.ge accept succeeds, and only post for targets 2 and 8. For a posted original at target 7 the payment guard in `_rs_auto_cancel_after_rs_cancellation` already applies.

### 38. Reject and Accept-cancellation paths ignore reversal_move_ids / payment_state 'reversed'; auto-cancel strands a reconciled credit note [C]
- **Module.** `rs_einvoice (buyer)`
...
- **2026-09-25.** The payment guard now sits inside `_rs_auto_cancel_after_rs_cancellation`, so a credit note reconciled with the bill (`payment_state` 'reversed') is no longer unreconciled on any path. Still open: a draft credit note, or a posted one not reconciled with the bill: the bill is cancelled and the credit note is left behind.
- **Fix.** Extend the payment guard to the move's reversals: in action_rs_buyer_accept (target 7) and _rs_do_buyer_reject skip the automatic cancel when payment_state != 'not_paid' or reversal_move_ids is set, and leave the manual step to the accountant (credit note already exists in this case: cancel it with the bill or keep both). Reuse the guard already present in action_rs_refresh_status and the recovery cron so all four entry points agree.

### 39. Bills created from rs.ge (inbox, cron, pull) carry no purchase order link; PO stays 'To Bill' beside a posted bill [C]
- **Module.** `rs_einvoice (buyer)`
...
- **Fix.** After the bill is created and before _rs_auto_import_pulled_to_bill_lines, run the core matcher (_find_and_set_purchase_orders with the partner and rs.ge total, or expose purchase_vendor_bill_id on the inbox row) so lines come from the PO with purchase_line_id, then apply rs.ge qty/price via _rs_merge_pulled_to_bill_lines (it matches on the rs.ge line ID, then on the name in order); fall back to bare lines only when no PO matches.

### 40. Any credit note (draft, cancelled, Skip-rs.ge partial) permanently disables the local cancel at rs.ge status 7 and 8 - silently, no TODO [C]
...
### 41. Auto-cancel of a prior-period original rewrites an already declared VAT period (no lock dates in live DB) [C]
- **Module.** `rs_einvoice (seller)`
...
### 43. Sent (Pending) invoice reset to draft, buyer confirms meanwhile, re-post succeeds with edited lines; rs.ge keeps the confirmed old lines [C]
- **Module.** `rs_einvoice (seller)`
...
### 44. rs.ge status 7 written by _rs_apply_status_update never triggers the local cancel [C]
- **Module.** `rs_einvoice (seller)`
...
### 46. Reimport Lines strips purchase_line_id from PO-created bill lines; PO drops to Waiting Bills while the bill stays posted [C]
...
### 51. Bill pulled from rs.ge is never linked to the PO the module built from the same waybill [C]
- **Module.** `rs_waybill + rs_einvoice (purchase bridge)`
- **Mechanism.** [Certain] rs_waybill builds and receives the PO from the vendor's waybill; rs_einvoice imports the vendor's e-invoice as bare bill lines (name/qty/price/tax only). Neither side looks the other up (rs_base_methods' waybill↔invoice link is metadata, and its invoice wizard is seller-side, known #29). The bill therefore never sets purchase_line_id, and the PO never sees the bill. [Likely] this is the default path for a vendor who files both a waybill and an e-invoice on rs.ge.
...
not had the same verification pass. #37, #38, #40, #41 and #44 all change the auto-cancel
decision (root design note) and belong in one design.
